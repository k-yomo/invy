...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	for name, query := range aq.withNamedUsers {
		if err := aq.loadUsers(ctx, query, nodes,
			...
			func(n *Account, e *User) { n.appendNamedUsers(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (aq *AccountQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Account, init func(*Account), assign func(*Account, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(account.UsersColumn, fks...))
	}))
...
func (aq *AccountQuery) WithNamedUsers(name string, opts ...func(*UserQuery)) *AccountQuery {
	query := &UserQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
