...
func Constraint(ctx context.Context, obj interface{}, next graphql.Resolver, min *int, max *int, pattern *string) (interface{}, error) {
	v, _ := next(ctx)

	switch value := v.(type) {
	case string:
		if err := stringConstraint(value, min, max, pattern); err != nil {
			return nil, err
		}
	case *string:
		if value != nil {
...
func stringConstraint(value string, min *int, max *int, pattern *string) error {
