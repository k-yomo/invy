...
// QueryLineAuth chains the current query on the "line_auth" edge.
func (uq *UserQuery) QueryLineAuth() *LineAuthQuery {
	query := &LineAuthQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withLineAuth:    uq.withLineAuth.Clone(),
...
// WithLineAuth tells the query-builder to eager-load the nodes that are connected to
// the "line_auth" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithLineAuth(opts ...func(*LineAuthQuery)) *UserQuery {
	query := &LineAuthQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withLineAuth = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withLineAuth; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.withFKs = true
		query.Where(predicate.LineAuth(func(s *sql.Selector) {
			s.Where(sql.InValues(user.LineAuthColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_line_auth
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_line_auth" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_line_auth" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.LineAuth = n
		}
	}
