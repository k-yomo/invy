...
// HasLineAuth applies the HasEdge predicate on the "line_auth" edge.
func HasLineAuth() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(LineAuthTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2O, false, LineAuthTable, LineAuthColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasLineAuthWith applies the HasEdge predicate on the "line_auth" edge with a given conditions (other predicates).
func HasLineAuthWith(preds ...predicate.LineAuth) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(LineAuthInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2O, false, LineAuthTable, LineAuthColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
